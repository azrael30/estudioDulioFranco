...
<body>
	...
	<?php
		...
		$sql = "UPDATE cliente SET apellido = '$apellido', celular = '$celular', companiaCelular = '$compania', CUIT = '$cuitSQL', fechaNacimiento = STR_TO_DATE('$fechaNacimiento', '%d/%m/%Y'), mail = '$mail', nacionalidad = '$nacionalidad', nombre = '$nombre', telefonoFijo = '$telefonoFijo' WHERE idCliente = '$idCliente'";
			...
			foreach ($_POST["obligaciones"] as $idObligacion) {
				...
				if(mysqli_num_rows($resultadoCheck)==0){//la obligacion no esta asociada al cliente
					...
					if($valorFijo!= ""){
						...
						$sqli = "UPDATE obligacioncliente SET habilitado = 1, valor = '$valorFijo' WHERE idCliente = '$idCliente' AND idObligacion = '$idObligacion'";
					}else{
